...
The following code is to calculate recombination rate for a set number of snps (simsnps) randomly placed on the GED chromosomes. 
We take the distance between SNPs (if on the same chromosome) and calculate a recombination rate depending on the distance 1cM per 1 million bases). If on different chromosomes we set the recombination rate between SNPs as "0.5". 
...
The next bit is to create a slim template that allows us to run our simulation scripts with several parameters. 
